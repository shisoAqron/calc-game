...
- **エンドレスモード** … 3回まちがえるまで続く。ライフ（❤️／💔）を問題の上に表示。リザルトはスコアとせいかい数（同じ大きさ）を表示し、トロフィー＋「がんばったね！」
- **モード**（一部予定）… エンドレスは実装済み。百マスは未実装で、タップすると「きのうついかをまっててね」とお知らせを表示。むずかしさ × モード を組み合わせて遊べる
